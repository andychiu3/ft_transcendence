(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const s of o)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(o){const s={};return o.integrity&&(s.integrity=o.integrity),o.referrerPolicy&&(s.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?s.credentials="include":o.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function a(o){if(o.ep)return;o.ep=!0;const s=n(o);fetch(o.href,s)}})();const Ae="modulepreload",Fe=function(e){return"/"+e},le={},ae=function(t,n,a){let o=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const i=document.querySelector("meta[property=csp-nonce]"),m=(i==null?void 0:i.nonce)||(i==null?void 0:i.getAttribute("nonce"));o=Promise.allSettled(n.map(l=>{if(l=Fe(l),l in le)return;le[l]=!0;const w=l.endsWith(".css"),Pe=w?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${Pe}`))return;const k=document.createElement("link");if(k.rel=w?"stylesheet":Ae,w||(k.as="script"),k.crossOrigin="",k.href=l,m&&k.setAttribute("nonce",m),document.head.appendChild(k),w)return new Promise((Te,Ce)=>{k.addEventListener("load",Te),k.addEventListener("error",()=>Ce(new Error(`Unable to preload CSS for ${l}`)))})}))}function s(i){const m=new Event("vite:preloadError",{cancelable:!0});if(m.payload=i,window.dispatchEvent(m),!m.defaultPrevented)throw i}return o.then(i=>{for(const m of i||[])m.status==="rejected"&&s(m.reason);return t().catch(s)})};async function ge(){try{const e=await fetch("/api/auth/verify",{credentials:"include"}),t=await e.json();return e.ok}catch(e){return console.error("JWT verification failed",e),!1}}async function Me(e){const t=e.get("email");if(!t){alert("enter email to get the code");return}try{const n=await fetch("/api/send2FACode",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t})}),a=n.json();n.ok?alert("code has been sent."):alert("fail to send")}catch{alert("got info which is not json?")}}function Ne(e){const t=document.getElementById("qrModal"),n=document.getElementById("qrImage"),a=document.getElementById("closeModalBtn");t&&n&&(n.src=e,t.classList.remove("hidden"),a&&(a.onclick=()=>{t.classList.add("hidden")}))}async function Oe(e){const t=e.get("email");if(!t){alert("Enter email for auth app check");return}try{const n=await fetch("/api/authAppSecretCheck",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t})});if(!n.ok){alert("please register first.");return}const a=await n.json();a.qrCodeDataUrl?Ne(a.qrCodeDataUrl):alert("unexpected case?")}catch{alert("catched err")}}function De(){const e=document.getElementById("sendCodeBtn"),t=document.getElementById("loginForm");e&&e.addEventListener("click",async()=>{const n=new FormData(t),a=n.get("method");a==="email"?await Me(n):a==="authApp"&&await Oe(n)})}async function je(){const e=document.getElementById("loginForm");e.addEventListener("submit",async t=>{t.preventDefault();const n=new FormData(e),a=n.get("email"),o=n.get("password"),s=n.get("code"),i=n.get("method"),m=await fetch("api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:a,password:o,authCode:s,method:i})}),l=await m.json();m.ok?(alert("login succeeded"),ae(()=>Promise.resolve().then(()=>Le),void 0).then(w=>w.navigate("/home"))):alert("login failed: "+String(l.error))})}async function $e(){var e;(e=document.getElementById("showTermsBtn"))==null||e.addEventListener("click",t=>{var n;t.preventDefault(),(n=document.getElementById("terms"))==null||n.classList.toggle("hidden")})}async function He(){if(await ge()){ae(()=>Promise.resolve().then(()=>Le),void 0).then(t=>t.navigate("/home"));return}De(),je(),$e()}const _e=e=>window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?e:`https://ft-transcendence-rtjp.onrender.com${e}`;function Re(){const e=document.getElementById("registerForm");e&&e.addEventListener("submit",async t=>{t.preventDefault();const n=new FormData(e),a=n.get("email"),o=n.get("password"),s=_e("/api/register");try{console.log(s);const i=await fetch(s,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:a,password:o})}),m=await i.json();i.ok?(alert("registration succeeded"),f("/login")):alert("registration failed: "+m.error)}catch{console.error("registration failed?"),alert("registration failed?")}})}function Ue(){var e;(e=document.getElementById("passwordForm"))==null||e.classList.remove("hidden")}function K(){var e;(e=document.getElementById("passwordForm"))==null||e.classList.add("hidden")}async function qe(){const e=document.getElementById("oldPassword").value,t=document.getElementById("newPassword").value;try{const n=await fetch("/api/profile/password",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({oldPassword:e,newPassword:t})}),a=await n.json();n.ok?alert("Password updated."):alert("Failed to update password: "+a.message),K()}catch{alert("catched err: failed to update password."),K()}}async function Je(){var e,t,n;(e=document.getElementById("showPasswordForm"))==null||e.addEventListener("click",Ue),(t=document.getElementById("submitPasswordChange"))==null||t.addEventListener("click",qe),(n=document.getElementById("hidePasswordForm"))==null||n.addEventListener("click",K)}function G(e){const t=document.getElementById("profileName"),n=document.getElementById("changeName"),a=document.getElementById("nameInput"),o=document.getElementById("saveName");e?(t.classList.add("hidden"),n.classList.add("invisible"),a.classList.remove("hidden"),o.classList.remove("hidden"),a.value=t.textContent||"",a.focus()):(a.classList.add("hidden"),o.classList.add("hidden"),t.classList.remove("hidden"),n.classList.remove("invisible"))}async function We(e){try{return(await fetch("/api/profile/name",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({newName:e}),credentials:"include"})).ok?(alert("name updated."),!0):(alert("name updating failed"),!1)}catch{return alert("name updating failed?"),!1}}function ze(){const e=document.getElementById("profileName"),t=document.getElementById("changeName"),n=document.getElementById("nameInput"),a=document.getElementById("saveName");t.addEventListener("click",()=>{G(!0)}),a.addEventListener("click",async()=>{const o=n.value.trim();if(o===""||o===e.textContent){G(!1);return}await We(o)&&(e.textContent=o),G(!1)})}function he(e){const t=document.getElementById("avatarImg");document.getElementById("avatarInput"),t.src=`/avatar/upload/${e}.png?${Date.now()}`,t.onerror=()=>{t.onerror=null,t.src="/avatar/42.svg"}}async function Ve(e,t){const n=new FormData;n.append("avatar",t);try{const a=await fetch("/api/avatar",{method:"POST",body:n}),o=await a.json();a.ok?(alert("avatar updated."),he(e)):alert(o.message||"avatar updating failed.")}catch{alert("Failed updating avatar?")}}async function Ge(e){document.getElementById("avatarImg");const t=document.getElementById("avatarInput");he(e.id),t.addEventListener("change",async()=>{var a;const n=(a=t.files)==null?void 0:a[0];if(n){if(n.size>5*1024*1024){alert("img limitation: 5MB");return}console.log("user id: ",e.id),await Ve(e.id,n)}})}function ye(e){const t=D(e);return t&&t.isFriend?t.online?"🟢":"🔴":"-"}function Qe(e){return D(e).isFriend?"bg-gray-400 cursor-not-allowed":"bg-blue-500 hover:bg-blue-600"}function X(e){return D(e).isFriend?"added":"add"}function Ye(e){const t=document.getElementById(`onlineStatus${e}`);t&&D(e).isFriend&&(t.textContent=ye(e))}function Ke(e){const t=document.getElementById(`addFriend${e}`);if(t){t.textContent=X(e);const n=D(e).isFriend;t.disabled=n,n?(t.classList.remove("bg-blue-500","hover:bg-blue-600"),t.classList.add("bg-gray-400","cursor-not-allowed")):(t.classList.add("bg-blue-500","hover:bg-blue-600"),t.classList.remove("bg-gray-400","cursor-not-allowed"))}}function Q(e){D(e)&&(Ke(e),Ye(e))}function Xe(){document.querySelectorAll('[id^="addFriend"]').forEach(e=>{const t=Number(e.id.replace("addFriend",""));e.addEventListener("click",()=>{ae(async()=>{const{addFriend:n}=await Promise.resolve().then(()=>et);return{addFriend:n}},void 0).then(({addFriend:n})=>n(t))})})}const I={},oe=()=>{const e=window.location.protocol==="https:"?"wss:":"ws:",t=window.location.host;return`${e}//${t}/ws`};console.log(oe());let c=new WebSocket(oe());function be(){(!c||c.readyState===WebSocket.CLOSED)&&(c=new WebSocket(oe())),c.onopen=()=>console.log("ws connected"),c.onclose=()=>console.log("ws closed"),c.onerror=e=>console.log("websocket error: ",e),c.onmessage=e=>{if(c.readyState!==WebSocket.OPEN){console.log("ws yet to be ready");return}const t=JSON.parse(e.data);console.log(t),t.method==="friendAdded"?(I[t.id]?I[t.id].isFriend=!0:I[t.id]={id:t.id,isFriend:!0,online:!1},Q(t.id)):t.method==="onlineStatus"?(I[t.id]?I[t.id].online=t.online:I[t.id]={id:t.id,isFriend:!1,online:t.online},Q(t.id)):t.method==="isFriend"&&(I[t.id]?I[t.id].isFriend=t.isFriend:I[t.id]={id:t.id,isFriend:t.isFriend,online:!1},Q(t.id))}}function Ze(e){c&&c.readyState===WebSocket.OPEN&&c.send(JSON.stringify({method:"addFriend",targetId:e}))}function ve(e){c&&c.readyState===WebSocket.OPEN&&c.send(JSON.stringify({method:"checkFriendship",targetId:e}))}function we(e){c&&c.readyState===WebSocket.OPEN&&c.send(JSON.stringify({method:"checkOnline",id:e}))}function D(e){return I[e]||{id:e,isFriend:!1,online:!1}}function xe(){c&&c.readyState===WebSocket.OPEN&&c.close()}const et=Object.freeze(Object.defineProperty({__proto__:null,addFriend:Ze,checkFriendship:ve,checkUserOnline:we,connectWebSocket:be,disconnectWebSocket:xe,getFriendStatus:D},Symbol.toStringTag,{value:"Module"}));async function Ee(){try{(await fetch("/api/logout",{method:"POST",credentials:"include"})).ok?(xe(),f("/login")):alert("logout failed")}catch{alert("logout failed")}}async function tt(){var e;(e=document.getElementById("deleteAccount"))==null||e.addEventListener("click",async()=>{try{(await fetch("/api/account",{method:"DELETE",credentials:"include"})).ok?(alert("account deleted. logging out..."),Ee()):alert("failed to delete account")}catch{alert("failed to delete account?")}})}function z(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function nt(e){const t=document.getElementById("main-content");t.innerHTML=`
	...
	`}function st(e){return e.map(n=>{const a=ye(n.id),o=Qe(n.id),s=X(n.id);return`
			<tr class="hover:bg-gray-50">
			...
			`}).join("")}async function rt(e){const t=document.getElementById("main-content");if(e.length===0){t.innerHTML='<h2 class="p-8"> no other users </h2>';return}e.forEach(a=>{we(a.id),ve(a.id)});const n=st(e);t.innerHTML=`
	<div class="flex-1 min-h-screen">
		<table class="w-full border-collapse text-sm">
			<thead>
				<tr class="bg-gray-200">
	...
	`,Xe()}async function it(){try{const e=await fetch("/api/profile",{credentials:"include"}),{ok:t,data:n}=await e.json();e.ok?nt(n):(alert(String(n.error)),f("/login"))}catch{alert("Pass may have expired. Please login again"),f("/login")}}async function lt(){try{const e=await fetch("/api/match/history",{credentials:"include"});if(e.ok){const t=await e.json();ot(t)}else alert("Pass may have expired. Please login again"),f("/login")}catch{alert("Pass may have expired. Please login again?"),f("/login")}}async function dt(){try{const e=await fetch("/api/users",{credentials:"include"});if(e.ok){const t=await e.json();rt(t)}else alert("Pass may have expired. Please login again"),f("/login")}catch{alert("Pass may have expired. Please login again?"),f("/login")}}const de={profile:it,matchHistory:lt,users:dt};async function ce(){var t,n,a;await ge()?(be(),(n=document.getElementById("sidebar"))==null||n.classList.remove("hidden"),(a=document.getElementById("main-content"))==null||a.classList.add("ml-64"),document.getElementById("logout-button").addEventListener("click",Ee),document.querySelectorAll("[data-tab]").forEach(o=>{o.addEventListener("click",async s=>{s.preventDefault();const i=o.getAttribute("data-tab");i&&de[i]&&await de[i]()})})):(document.querySelector("#login-logout a[data-link]").classList.remove("hidden"),(t=document.getElementById("main-content"))==null||t.classList.remove("ml-64"))}var u,p;const B={w:!1,s:!1,ArrowUp:!1,ArrowDown:!1};var F=0,M=0,L,A="",Ie=0,Be=0,_,j,V,E,H,S,Z,R,U,se,b,$,v,ee,T,C,d,te,x,P,h,g,N,O,q,y=null;function W(){p.fillStyle="black",p.fillRect(0,0,u.width,u.height),p.strokeStyle="white",p.beginPath(),p.setLineDash([5,15]),p.moveTo(u.width/2,5),p.lineTo(u.width/2,u.height),p.stroke(),p.fillStyle=Z,p.fillRect(H,S,V,E),p.fillStyle=ee,p.fillRect($,v,se,b),p.fillStyle="white",p.fillRect(h,g,d,d),_&&N&&O&&(p.fillStyle="red",p.fillRect(N,O,q,q))}function J(e,t,n){return n>0&&e<u.height-t?!0:n<0&&e>0}function ct(){B.w&&J(S,E,R)?S+=R:B.s&&J(S,E,U)&&(S+=U),B.ArrowUp&&J(v,b,T)?v+=T:B.ArrowDown&&J(v,b,C)&&(v+=C)}async function ut(){const t=Math.abs($-(h+d))/Math.abs(x);let n=g+d+P*t,a=g+P*t;if(a<0?(a=0,n=d):n>u.height&&(n=u.height,a=u.height-d),n>v+b){const o=Math.abs(v+b-n),s=Math.ceil(o/Math.abs(C))*(1e3/60);B[C>0?"ArrowDown":"ArrowUp"]=!0,setTimeout(()=>{B[C>0?"ArrowDown":"ArrowUp"]=!1},s+100)}else if(a<v){const o=Math.abs(v-a),s=Math.ceil(o/Math.abs(T))*(1e3/60);B[T<0?"ArrowUp":"ArrowDown"]=!0,setTimeout(()=>{B[T<0?"ArrowUp":"ArrowDown"]=!1},s+100)}}function ue(e,t,n){return((t+d/2-e)/n-.5)*5}function mt(){h+=x,g+=P,h<0&&(M++,document.getElementById("player2Score")&&(document.getElementById("player2Score").textContent=M.toString()),ne()),h+d>u.width&&(F++,document.getElementById("player1Score")&&(document.getElementById("player1Score").textContent=F.toString()),ne()),(g<0||g+d>u.height)&&(P=-P),h<H+V&&h+d>H&&g+d>S&&g<S+E&&(x=-x,P=ue(S,g,E),h=H+V+1,me()),h+d>$&&h<$+se&&g+d>v&&g<v+b&&(x=-x,P=ue(v,g,b),h=$-d-1,me()),_&&h+d>N&&h<N+q&&g+d>O&&g<O+q&&(N=0,O=0,pt(x))}function me(){Math.random()<.25&&_&&!N&&!O&&(N=Math.random()*(u.width-150-150)+150,O=Math.random()*(u.height-100-100)+100)}function pt(e){const t=e>0?1:-1,n=Math.random();n<.25?(document.getElementById("powerupInfo").textContent="Paddle size increased!",t==1&&E<200?E+=25:b<200&&(b+=25)):n<.5?(document.getElementById("powerupInfo").textContent="Opponent paddle shrunk!",t==1&&b>20?b-=7:E>20&&(E-=7)):n<.75?(document.getElementById("powerupInfo").textContent="Opponent control reversed!",t==1?(T=-T,C=-C):(R=-R,U=-U)):(document.getElementById("powerupInfo").textContent="Ball size increased!",d<50&&(d+=5))}async function ft(e){try{(await fetch("/api/match-result",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})).ok||alert("failed saving match result")}catch{alert("failed saving match result?")}}async function gt(){if(!j)return;const e=A==="Player 1"?"win":"lose";try{if(!(await fetch("/api/stat",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({result:e})})).ok){console.log("would not update if not sign in");return}}catch{alert("didnt update stat versus AI")}}function ht(){if(ct(),mt(),W(),F==L||M==L)if(clearInterval(Ie),clearInterval(Be),y&&y.started&&y.matches){const e=y.matches.find(t=>t.winner===null);e&&(F==L?(e.winner=e.user1,A=e.user1):M==L&&(e.winner=e.user2,A=e.user2),y.matchnumber++,localStorage.setItem("tournament",JSON.stringify(y)),ft(y),F=0,M=0,document.getElementById("winnerInfo").innerHTML=`<p>${A} wins!</p>`,document.getElementById("winnerInfo").innerHTML+='<button id="finish-button" class="px-4 py-2 bg-white text-black rounded">Finish match</button>',document.getElementById("finish-button").addEventListener("click",()=>{f("/tournament")}))}else F==L?A="Player 1":M==L&&(A=j?"AI":"Player 2"),gt(),F=0,M=0,document.getElementById("winnerInfo").innerHTML=`<p>${A} wins!</p>`,document.getElementById("winnerInfo").innerHTML+='<button id="finish-button" class="px-4 py-2 bg-white text-black rounded">Finish match</button>',document.getElementById("finish-button").addEventListener("click",()=>{f("/")})}function Se(){V=15,E=75,H=25,R=-12,U=12,se=15,b=75,$=u.width-30,T=-12,C=12,d=10,te=10,q=35}function ne(){Se(),document.getElementById("powerupInfo").textContent="",h=u.width/2-d/2,g=u.height/2-d/2,x=-x,P=0,S=u.height/2-E/2,v=u.height/2-b/2}function yt(){var t,n,a,o,s,i,m;var e=localStorage.getItem("tournament");if(e?y=JSON.parse(e):y=null,u=document.getElementById("pongCanvas"),p=u.getContext("2d"),Se(),x=te,(t=document.getElementById("ballSpeed"))==null||t.addEventListener("input",()=>{const l=document.getElementById("ballSpeed").valueAsNumber;x=te*(l/100),document.getElementById("speedText").textContent=`Ball Speed: (${l}%)`}),L=5,(n=document.getElementById("winAmount"))==null||n.addEventListener("input",()=>{const l=document.getElementById("winAmount").valueAsNumber;L=l,document.getElementById("winAmountText").textContent=`Winning Score: (${l})`,document.getElementById("winAmountInfoText").textContent=`First to ${l} points wins`}),Z="#ffffff",ee="#ffffff",(a=document.getElementById("paddle1Color"))==null||a.addEventListener("input",()=>{Z=document.getElementById("paddle1Color").value,W()}),(o=document.getElementById("paddle2Color"))==null||o.addEventListener("input",()=>{ee=document.getElementById("paddle2Color").value,W()}),_=!0,(s=document.getElementById("powerupsEnabled"))==null||s.addEventListener("input",()=>{_=document.getElementById("powerupsEnabled").checked}),j=!1,(i=document.getElementById("aiEnabled"))==null||i.addEventListener("input",()=>{j=document.getElementById("aiEnabled").checked,document.getElementById("player2Name").textContent=j?"AI":"Player 2"}),y&&y.started&&y.matches){const l=y.matches.find(w=>w.winner===null);l&&(document.getElementById("player1Name").textContent=l.user1,document.getElementById("player2Name").textContent=l.user2,document.getElementById("match-title").textContent=`Tournament Match ${y.matchnumber}`,document.getElementById("aiEnabled").disabled=!0)}ne(),W(),(m=document.getElementById("startButton"))==null||m.addEventListener("click",()=>{const l=document.getElementById("startInfo");l&&(l.style.visibility="hidden"),document.addEventListener("keydown",function(w){B[w.key]=!0}),document.addEventListener("keyup",function(w){B[w.key]=!1}),Ie=setInterval(ht,1e3/60),j&&(Be=setInterval(ut,1e3))})}var r=null;function bt(){return`
	...
	`}function ke(){const e=document.getElementById("current-users");e.innerHTML="",r.users.forEach(n=>{const a=document.createElement("button");a.id=n,a.innerHTML=n,a.classList.add("block","p-2","text-gray-800","bg-gray-200","rounded-md","border-2","border-gray-200","hover:bg-red-200"),a.addEventListener("click",()=>{r.users=r.users.filter(o=>o!==n),ke()}),e.appendChild(a)});const t=document.getElementById("create-tournament-button");r.users.length>1?(t.disabled=!1,t.classList.remove("bg-gray-300"),t.classList.add("bg-gray-100"),t.classList.add("hover:bg-gray-200")):(t.disabled=!0,t.classList.remove("bg-gray-100"),t.classList.remove("hover:bg-gray-200"),t.classList.add("bg-gray-300"))}function pe(){r.matches=[];for(let e=0;e<r.users.length;e+=2)e+1<r.users.length?r.matches.push({user1:r.users[e],user2:r.users[e+1],winner:null}):r.matches.push({user1:r.users[e],user2:"BYE",winner:r.users[e]});localStorage.setItem("tournament",JSON.stringify(r))}function wt(){var e=localStorage.getItem("tournament");if(e?r=JSON.parse(e):r=null,r&&r.started&&r.matches){r.matches.every(n=>n.winner!==null)&&(r.users=r.matches.map(n=>n.winner),pe()),r.users.length===1&&(alert("tournament done, winner is "+r.users[0]),localStorage.removeItem("tournament"),f("/")),document.getElementById("tournament-main").innerHTML=vt();const t=r.matches.find(n=>n.winner===null);document.getElementById("match-title").innerHTML=`Match ${r.matchnumber}`,document.getElementById("match-user1").innerHTML=t.user1,document.getElementById("match-user2").innerHTML=t.user2,document.getElementById("match-start").addEventListener("click",()=>{f("/play")}),document.getElementById("end-tournament").addEventListener("click",()=>{alert("tournament ended early"),localStorage.removeItem("tournament"),f("/")})}else r={users:[],matches:[],matchnumber:1,started:!1},localStorage.setItem("tournament",JSON.stringify(r)),document.getElementById("tournament-main").innerHTML=bt(),document.getElementById("add-user-button").addEventListener("click",()=>{const t=document.getElementById("add-user-input"),n=t.value;n&&!r.users.includes(n)&&(r.users.push(n),t.value="",ke())}),document.getElementById("create-tournament-button").addEventListener("click",()=>{r.started=!0,pe(),f("/tournament")})}function xt(){return`
	...
	`}const kt={"/":fe,"/login":xt,"/register":Et,"/home":fe,"/play":It,"/404":Bt,"/tournament":St},Y={"/":ce,"/login":He,"/register":Re,"/home":ce,"/play":yt,"/tournament":wt};function re(e){var n;const t=kt[e];t||f("/404"),document.body.innerHTML=t(),(n=Y[e])==null||n.call(Y)}function ie(e){const t={"/":"Home","/login":"Login","/register":"Register","/play":"Pong","/tournament":"Tournament"};document.title=t[e]||"ft_transcendence"}function f(e){history.pushState({},"",e),ie(e),re(e)}function Lt(){document.body.addEventListener("click",e=>{const t=e.target;if(t.matches("[data-link]")){e.preventDefault();const n=t.getAttribute("href");n&&f(n)}})}window.addEventListener("load",()=>{re(location.pathname),ie(location.pathname),Lt()});window.addEventListener("popstate",()=>{re(location.pathname),ie(location.pathname)});const Le=Object.freeze(Object.defineProperty({__proto__:null,navigate:f},Symbol.toStringTag,{value:"Module"}));
